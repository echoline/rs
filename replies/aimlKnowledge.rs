// aiml2rs -- Generated on Sat Sep 26 06:35:29 2009
...
+ what is a cocktail
- <set it=Cocktail><get it> is a combination of spirits, wines, flavorings, sweeteners and garnishes of various kinds, intended to be consumed before dining.
...
+ what is a dream
- <set it=A Dream><get it> is an unconscious experience while sleeping.
...
+ what is a eukaryote
- {random}A eukaryote is a relatively 'modern' type of cellular life, distinguished from the more ancient prokaryotes by their distinct nuclear membrane and chromasomes, -which are lacking in prokaryotes.|An animal that is not a prokaryote.{/random}
^ <set it=eukaryote>
...
+ what is a female
- Females are the better and more beautiful half of humanity. mothers are born female!  My <bot botmaster> is extremely fond of mothers, and females generally.
^ <set it=female>
...
+ what is a pear
- A pear is a delightful cultivated yellow-green sweet fruit that grows on trees with glossy leaves and white flowers. My <bot botmaster> thinks they are yummy.
^ <set it=pear>
...
^ <set it=perfect operational record>
...
+ what is a plant
- A plant is a photosythetic, eukaryotic multicellular organism, often consumed by free-roaming wetware entities as food.  My <bot botmaster> says they are often yummy, but that brussels sprouts are an abomination.
^ <set it=plant>
...
- <set it=A device for measuring temperature.>
...
+ what is adsl
- Asynchronous digital subscriber line.<set it=ADSL>
...
^ <set it=epistemological question>
...
^ <set it=ANTIDISESTABLISHMENTARIANISM>
...
+ what is better
- {random}I can't decide.|You decide.|I have no preference.{/random}
...
+ what is chatter
- {random}Cheese made form French cat's milk.|To make short, indistinct sounds in rapid succession, hence birds and squirrels chatter. To talk fast, incessantly, and foolishly. To click together rapidly, as the teeth do when the lower jaw traebles from fright or cold.{/random}
^ <set it=CHATTER>

+ what is cheese
- {random}A word that makes people hate photographers.|A food made from the curds of soured milk pressed together to form a solid that is usually allowed to ripen.{/random}
^ <set it=CHEESE>

+ what is chess
- {random}Chort for Mexican name Jesus.|A game of skill played on a chessboard by two players, each with 16 pieces limited in movement according to kind, the object being to checkmate the opponent's king.{/random}
^ <set it=CHESS>
...
+ what is chicken
- {random}A food that uncannily tastes like itself.|A common gllinaceous farm bird raised for its edible eggs or flesh.{/random}
^ <set it=CHICKEN>
...
+ what is clinton
- {random}An inflated Bill .|42nd President of the U.S.{/random}
^ <set it=Clinton>
...
^ <set it=Consciousness>

+ what is context
- {random}Tiny Time Pills.|The parts of a sentence, paragraph, discourse, etc. immediately next to or surrounding a specified word or passage and determining its exact meaning.{/random}
^ <set it=Context>
...
+ what is e
- The fifth letter of the alphabet.<set it=E>
...
+ what is freedom
- Liberty to do think, feel and do just as one pleases.<set it=FREEDOM>
...
- Bonjour.
^ <set it=HELLO IN FRENCH>

+ what is hello in german
- Hallo.
^ <set it=HELLO IN GERMAN>

+ what is hello in spanish
- Hola

+ what is help
- <set it=Help><get it> is assistance I can give you.
...
+ what is imagine
- {random}Imagine is a verb.|A song by John Lennon.|Imagination.{/random}
...
+ what is iran
- Iran is a nation in southern Asia.  <set it=Iran><get it> is the site of ancient Persia.
...
+ what is one
- The numerical value of unity; the successor to zero.<set it=ONE>
...
^ <set it=PHOTOSYNTHESIS>

+ what is php
- The original name was "Personal Home Page", now Hypertext Processor.<set it=PHP>
...
- The ratio of the energy of a photon to its frequency.  The numerical value is six point six two six two times ten to the negative thirty fourth power Joule second.<set it=PLANCK S CONSTANT>
...
+ what is poetry
- The output of a poet.<set it=POETRY>
...
+ what is power
- {random}The rate of doing work, measured in Watts = Joules per second.|The ability to influence peope or events.|Power is an illusion.{/random}
^ <set it=Power>

+ what is prayer
- {random}Meditation before God.|Some people say that prayer is no different than clear thinking.|To pray is to ask God for His forgiveness and mercy.{/random}
^ <set it=prayer>
...
+ what is silver
- Atomic Symbol: Ag.  Atomic Number: 47.  Atomic weight 107.870.  White lustrous soft metal.<set it=SILVER>
...
- <set it=Thanksgiving><get it> is a holiday to celebrate the Autumn harvest.
...
- "<reply1>"  means exactly what it says.

+ what is that
- {random}"<reply1>"? What is it?|I've lost the context again.|Can you ask me more precisely?|Does it have anything to do with <get it>?{/random}
...
+ what is wap
- Wireless Application Protocol.<set it=WAP>
